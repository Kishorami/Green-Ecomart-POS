...
namespace App\Http\Livewire\Reports\DueSellsReport;
...
class DueSellsReportComponent extends Component
{
    public $reports;
    public $from, $to, $flag='';
    public $temp_from, $temp_to;
    public $sell_total, $profit_total, $due_total;

    public $sell, $bill_no, $profit, $due;
    ...
    public function search_by_date()
    {
        ...
        $this->reports = Sell::whereBetween('created_at', [$this->from, Carbon::parse($this->to)->addDays(1)])->where('due','>', 0)->with('customer')->get();
        ...
        if ($this->reports) {
            foreach ($this->reports as $key => $value) {
                ...
                $this->due_total = $this->due_total + $value->due;
            }
        }
    }
    ...
    public function get_item($id)
    {
        ...
        $this->due = $this->sell->due;

    }
    public function render()
    {
        return view('livewire.reports.due-sells-report.due-sells-report-component')->layout('base.base');
    }
}
